...
// --- checkAuth (sin reserva de números) ---
function checkAuth() {
...
// --- Guardar Orden (GENERA NÚMERO SOLO AL GUARDAR) ---
    ...
    orderForm.addEventListener('submit', async (e) => {
        ...
        try {
            ...
            if (currentEditOrderId) {
                // --- EDITAR ORDEN EXISTENTE (mantiene el mismo número) ---
                const orderRef = ref(db, `ordenes/${currentEditOrderId}`);
                
                // Obtener la orden original para mantener el número
                const originalOrder = ordersData[currentEditOrderId];
                
                const orderData = {
                    numeroOrden: originalOrder.numeroOrden, // Mantener el mismo número
                ...
                // --- CREAR NUEVA ORDEN (GENERAR NÚMERO AQUÍ) ---
                const counterRef = ref(db, 'metadata/lastOrderNumber');
                
                // Transacción atómica para obtener y actualizar el contador
...
window.resetOrderForm = function() {
    ...
    // Mostrar "---" en lugar de un número reservado
...
window.editOrder = function(orderId) {
    ...
    // Mostrar el número de la orden que se está editando
    const displayEl = document.getElementById('next-order-display');
