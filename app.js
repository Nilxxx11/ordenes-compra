...
// --- Manejo de Items (AHORA INCLUYE CENTRO DE COSTO) ---
...
// --- Guardar Orden (CON NUEVOS CAMPOS) ---
    ...
    orderForm.addEventListener('submit', async (e) => {
        ...
        try {
            ...
            if (currentEditOrderId) {
                ...
                const originalOrder = ordersData[currentEditOrderId];
                
                const orderData = {
                    numeroOrden: originalOrder.numeroOrden,
                    fecha: originalOrder.fecha,
                    comprador: originalOrder.comprador || {
                    ...
                    totales: totals,
                    estado: originalOrder.estado || 'ACTIVA',
                    creadoPor: originalOrder.creadoPor || {
                        uid: currentUser.uid,
                ...
                const result = await runTransaction(counterRef, (currentVal) => {
                    const newNumber = (currentVal || 999) + 1;
                    return newNumber;
                });
...
window.editOrder = function(orderId) {
    ...
    const displayEl = document.getElementById('next-order-display');
    if (displayEl) displayEl.textContent = order.numeroOrden + ' (editando)';
...
// --- Ver Detalles de Orden (CON NUEVOS CAMPOS) ---
    ...
    const itemsTable = order.items && order.items.length > 0 
        ? order.items.map(item => `
            <tr style="border-bottom:1px solid #ddd;">
                ...
                <td style="padding:8px;">${item.centroCosto || ''}</td>
        ...
        `).join('')
        : '<tr><td colspan="6" style="padding:8px; text-align:center;">No hay items</td></tr>';
        ...
        <div style="padding:20px;">
            ...
            <div style="display:grid; grid-template-columns:1fr 1fr; gap:20px; margin-bottom:20px;">
                <div class="info-card">
                    ...
                    <p><strong>Tipo de Gasto:</strong> ${order.tipoGasto || 'N/A'}</p>
                    <p><strong>Autorizado por:</strong> ${order.autorizadoPor || 'No especificado'}</p>
                </div>
                <div class="info-card">
                    <h4>Fechas</h4>
                    <p><strong>Creación:</strong> ${order.fecha ? new Date(order.fecha).toLocaleString('es-CO') : 'N/A'}</p>
                    ${order.ultimaModificacion ? `<p><strong>Última modificación:</strong> ${new Date(order.ultimaModificacion).toLocaleString('es-CO')}</p>` : ''}
                </div>
            </div>
                ...
                <table style="width:100%; border-collapse:collapse;">
                    <thead>
                        <tr style="background:#0D47A1; color:white;">
                            ...
                            <th style="padding:8px;">Centro Costo</th>
            ...
            <div style="margin-top:20px; text-align:right; color:#666; font-size:0.9rem;">
                ...
                ${order.editadoPor ? `<p>Editado por: ${order.editadoPor.email} - ${new Date(order.editadoPor.fecha).toLocaleString('es-CO')}</p>` : ''}
            </div>
...
window.generatePDF = async function(orderId) {
    ...
    try {
        ...
        doc.text(`Autorizado por: ${order.autorizadoPor || 'No especificado'}`, 20, 55);
        
        doc.setFontSize(12);
        doc.setTextColor(13, 71, 161);
        doc.text('DATOS DEL PROVEEDOR', 20, 65);
        
        doc.setFontSize(10);
        doc.setTextColor(0, 0, 0);
        doc.text(`Razón Social: ${order.proveedor?.razonSocial || 'N/A'}`, 20, 73);
        doc.text(`NIT: ${order.proveedor?.nit || 'N/A'}`, 20, 80);
        if (order.proveedor?.direccion) doc.text(`Dirección: ${order.proveedor.direccion}`, 20, 87);
        if (order.proveedor?.telefono) doc.text(`Teléfono: ${order.proveedor.telefono}`, 20, 94);
        if (order.proveedor?.correo) doc.text(`Email: ${order.proveedor.correo}`, 20, 101);
        
        doc.text(`Tipo de Gasto: ${order.tipoGasto || 'N/A'}`, 20, 108);
        
        doc.setFontSize(12);
        doc.setTextColor(13, 71, 161);
        doc.text('DETALLE DE ÍTEMS', 20, 118);
        ...
        doc.setTextColor(0, 0, 0);
        doc.text('#', 22, y);
        doc.text('Descripción', 32, y);
        doc.text('Centro Costo', 80, y);
        doc.text('Cant.', 115, y);
        ...
        if (order.items && order.items.length > 0) {
            order.items.forEach((item, index) => {
                ...
                doc.text((item.descripcion || '').substring(0, 15), 32, y);
                doc.text((item.centroCosto || '').substring(0, 10), 80, y);
                doc.text((item.cantidad || 0).toString(), 118, y);
                doc.text('$' + (item.pUnit || 0).toLocaleString('es-CO'), 135, y);
                doc.text('$' + (item.total || 0).toLocaleString('es-CO'), 160, y);
...
// --- Exportar a Excel (CON NUEVOS CAMPOS) ---
        ...
        if (order.items && order.items.length > 0) {
            order.items.forEach(item => {
                exportData.push({
                    ...
                    'Rete ICA': '',
                    'Total': item.total,
                    'Creado Por': `Cant: ${item.cantidad} x $${item.pUnit.toLocaleString('es-CO')}`,
                    'Observaciones': ''
                });
            });
